var controller = (function() {
  ...
  function getInstanceText(values, oldValues, idx) {
    var value = "";
    if (
      values[idx].type === "string" &&
      values[idx].value !== "" &&
      ((oldValues[idx].value !== "" &&
        fadeLevel == 1 &&
        (((nowCorrect || threeTries) &&
          values[idx].name.replace("self.", "") === nowCorrectVar) ||
          values[idx].name.replace("self.", "") !== nowCorrectVar)) ||
        fadeLevel == 0)
    ) {
      ...
      if (
        ...
        ((oldValues[idx].value === "" && fadeLevel === 0) ||
          (oldValues[idx].value &&
            fadeLevel === 1 &&
            ((nowCorrect || threeTries) &&
              nowCorrectVar === v.name &&
              nowCorrectAnswerType === "variable_bank")))
      ) {
        return 1;
      }
    }
  ...
  function displayParamVal(d, i, vOldLocalVariables) {
    if (fadeLevel == 0) {
      ...
      vOldLocalVariables[i].value !== "" &&
      fadeLevel == 1 &&
      (((nowCorrect || threeTries) && nowCorrectVar === d.name) ||
        nowCorrectVar !== d.name)
    ) {
      return getValueRep(d);
    } else {
      return "";
  ...
  function visualizeObjectInVariableBank(variable, simpleVariableBank) {
    ...
    var vVariables = makeList(
      simpleVariableBank["current_vb"][variable]["variables"]
    );
    ...
    if (!isObjectEmpty(variableBankObject)) {
      for (var variable in variableBankObject) {
        ...
        if (
          ...
          if (
            (simpleVariableBank["previous_vb"].hasOwnProperty(variable) &&
              fadeLevel == 1 &&
              (((nowCorrect || threeTries) && nowCorrectVar === variable) ||
                nowCorrectVar !== variable)) ||
            (simpleVariableBank["current_vb"].hasOwnProperty(variable) &&
              fadeLevel == 0) ||
            simpleVariableBank["current_vb"][variable]["type"] === "temp"
          ) {
            if (
  ...
  function checkInstance() {
    var correctAnswerObject = simulatorInterface.getCorrectAnswer();
    var userAnswer = d3
      .select('input[name="class_multiple_choice_radio"]:checked')
      .node().value;
    var correctAnswer;
    for (let idx = 0; idx < correctAnswerObject.rhs.values.length; idx++) {
      //Hacky AF, but doesn't actually give us the line we were on...
      if (correctAnswerObject.rhs.values[idx].type === "instance") {
        correctAnswer =
          correctAnswerObject.rhs.values[idx].hidden_val.value.name;
      }
    }
